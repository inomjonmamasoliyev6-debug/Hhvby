...
TOKEN = os.environ.get("BOT_TOKEN_OVERRIDE") or "8856340901:AAErIjCS_dmoAUVRwtzajyA29_zlc_eNSDA"
...
def admin_panel_keyboard(user_id=None):
    ...
    if user_id == SUPER_ADMIN:   # bloklash va bildirishnoma tugmalari FAQAT egada ko'rinadi
        ...
        rows.insert(len(rows) - 2, [InlineKeyboardButton("🔔 Bildirishnomalar", callback_data="admin_toggle_events")])
...
async def handle_admin_text_state(update, context, user_id, text):
    ...
    if state not in ("add_channel", "set_price", "broadcast", "add_admin",
                     ...
                     "create_promo", "set_payment_channel", "ban_user", "unban_user", "find_user"):
        return False
    if text == CANCEL_TEXT:
        context.user_data["state"] = None
...
async def acquire_singleton():
    """PostgreSQL da: bot ikki joyda bir vaqtda ishlamasligi uchun (eng ko'pi 45 soniya kutadi)."""
    global _lock_conn, _singleton_note
    if not USE_PG or asyncpg is None:
        return
    try:
        conn = await asyncpg.connect(_pg_dsn())
    except Exception:
        logger.exception("singleton ulanish xato")
        return
    deadline = time.monotonic() + 45
    while True:
        try:
            ok = await conn.fetchval("SELECT pg_try_advisory_lock($1)", 7261993001)
        except Exception:
            logger.exception("singleton lock xato")
            ok = True
        if ok:
            _lock_conn = conn
            return
        if time.monotonic() > deadline:
            _lock_conn = conn
            _singleton_note = "\n⚠️ DIQQAT: botning BOSHQA NUSXASI hali ham ishlayapti (Render da ikkita xizmat yoki boshqa hosting/kompyuter)!"
            logger.warning("Boshqa nusxa ishlayapti")
            return
        await asyncio.sleep(3)
...
async def error_handler(update, context):
    ...
    if isinstance(err, Conflict):
        notify_owner(
            "⚠️ CONFLICT: botning boshqa nusxasi ham ishlayapti!\n\n"
            "Shu sababli ma'lumotlar chalkash ko'rinadi, xabar yuborish/kanal sozlamalari yo'qolib qoladi.\n"
            "Nima qilish kerak: Render'da bir xil botning 2 ta xizmati (Web Service + Worker) yoki boshqa hosting/kompyuterda "
            "ishlab turgan nusxani o'chiring. Faqat bittasi qolsin.\n"
            f"Bu nusxa: {INSTANCE_ID}", key="conflict", ttl=900)
        return
...
def build_application():
    ...
    try:
        class SafeBot(ExtBot):
            """Markdown xato bo'lsa xabarni yo'qotmaydi: oddiy matn qilib qayta yuboradi."""
...
def main():
    ...
    app.add_handler(CommandHandler("holat", status_command))
