...
async def _send_one(bot, uid, text, markdown=True):
    """'ok' | 'blocked' | 'failed'"""
    plain = not markdown
    for _ in range(4):
        try:
...
async def run_broadcast(bot, admin_chat, user_ids, text, markdown=True):
    total = len(user_ids)
    ...
    for i in range(0, total, BATCH):
        ...
        results = await asyncio.gather(*[_send_one(bot, u, text, markdown) for u in chunk], return_exceptions=True)
        for r in results:
